...
- The project is Next.js 15 App Router + React 19 + TypeScript strict + Tailwind CSS v3.
...
```dot
digraph when {
  "Premium visual redesign?" -> "Existing Next.js + React + Tailwind?" [label="yes"];
  "Existing Next.js + React + Tailwind?" -> "Use this skill" [label="yes"];
  "Existing Next.js + React + Tailwind?" -> "Use general frontend redesign" [label="no"];
...
- **Focus rings:** set `focus:border-accent focus:ring-2 focus:ring-accent/30` on inputs (update `OrderForm` classes).
- **Gradient button:** `bg-accent` base, `hover:bg-accent-hover`, and a subtle `shadow-lg shadow-accent/25`.
