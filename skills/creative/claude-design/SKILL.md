...
## Related Skills / Before You Start
...
When designing for an **existing project repo** (e.g. Penta Junior v2), also load any project-specific reference notes the skill carries — see `references/pentajunior-v2-product-design.md` for pulling real product data, images, and brand tokens from that catalog.

For evaluating external skill libraries that promise design capabilities (e.g. Google Stitch Skills), see `references/external-skill-evaluation.md`. The usual rule: do not adopt a dependency on an external MCP/service unless it provides something we genuinely cannot replicate locally. Harvest the useful ideas instead.
...
Before producing any design artifact, run the user's brief through this
pipeline. The goal is to replace vague, generic, or emotionally loaded terms
with precise, professional design language and to surface hidden decisions.

**This pipeline must not replace the existing Workflow below** — it feeds
into it. Apply it immediately after "Understand the brief" and before
"Gather context" or "Define the design system."

### Step 1 — Detect loaded or vague terms

Flag words that carry no concrete visual information:

- nice, cool, modern, clean, professional, beautiful, sleek
- simple, minimal, user-friendly, intuitive, premium (without specifics)
- "make it pop", "add some flair", "something fresh"
- generic section names: "Features", "Solutions", "About Us" (without content)

### Step 2 — Ask one or two clarifying questions only if needed

If the brief is missing any of the following, ask briefly:

- intended output format (HTML file, repo component, deck, prototype)
- audience / who will use it
- fidelity level
- brand/design system or reference site
- number of variations
- conservative vs. divergent direction
- whether a known brand look is desired (Stripe, Linear, Vercel, etc.)

Skip questions if the user already supplied enough direction.

### Step 3 — Load companion skills before defining the system

If the user wants a known brand look, load `popular-web-designs` and use its
templates for exact tokens. If the user asks for style/color/font direction or
says "help me design a ...", load `ui-ux-pro-max` first to get a data-driven
brief. Use those as the visual vocabulary, then run this enhancement on top.

### Step 4 — Translate vague terms to professional design language

Use this mapping to rewrite the brief into concrete design decisions:

| Vague user term | Professional design language |
|---|---|
| "make it nice" | "refine visual hierarchy, whitespace, and type rhythm" |
| "modern" | "generous whitespace, precise type scale, minimal chrome" |
| "clean" | "high contrast, clear information scent, reduced decorative noise" |
| "premium / luxury" | "restrained palette, elevated type, generous spacing, subtle motion" |
| "professional" | "clear hierarchy, accessible contrast, consistent component treatment" |
| "make it pop" | "one high-contrast accent used sparingly on primary actions" |
| "simple" | "single primary action per surface, reduced cognitive load" |
| "intuitive" | "familiar interaction patterns, clear affordances, consistent feedback" |
| "futuristic" | "monospace accents, dark surfaces, subtle glow, geometric layout" |
| "friendly" | "rounded corners, warm neutrals, approachable type, soft shadows" |
| "technical" | "mono type for code/data, dense information, keyboard affordances" |
| "organic" | "warm palette, soft shapes, editorial serif, hand-tuned spacing" |

### Step 5 — Structure the final brief

Rewrite the brief as 4–6 bullets:

- **Purpose:** what the artifact must communicate or enable
- **Audience:** who will view/use it
- **Format:** HTML file / repo page / deck / prototype / component lab
- **Visual posture:** density, color mode, motion level, brand reference
- **Key decisions:** layout model, type system, component primitives
- **What to avoid:** anti-patterns from the Anti-Slop Rules below

Use this enhanced brief to drive the rest of the Workflow. Do not show the
raw pipeline output to the user unless they ask for it; present the result as
...
2. **Enhance the brief**
   ...
   - Load `popular-web-designs` or `ui-ux-pro-max` if they provide useful
     visual vocabulary.
...
### Delivery hygiene (CLI mode)
When building multi-file deliverables on CLI, the user sees raw tool output in the terminal.
**Do not** silently write multi-thousand-line files and dump raw escapes at them.
**Do** tell them what is happening in plain language BEFORE writing:
  - "I will create a dark-themed landing page: one self-contained folder with `index.html`, `style.css`, and `script.js`."
**Do** keep file-write outputs out of conversation by calling `write_file` without narrating every line. The tool output is for the agent, not the user.
**Do** after all files are written, give a brief human-readable summary with exact file paths and how to open them.
**Do** handle delivery failure gracefully (e.g. messaging timeout). After 2 failed attempts, stop retrying, give the user exact local paths, and let them copy manually rather than spamming retries and losing their attention.

### Skill-loading pitfall
`sketch` is an ambiguous skill name in this environment (it matches both `creative/sketch/SKILL.md` and a file under `baoyu-article-illustrator/references`). Load it explicitly by full path as `creative/sketch` if you need throwaway HTML mockup variants.
